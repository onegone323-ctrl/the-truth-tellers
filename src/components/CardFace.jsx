...
// Luxury tarot card — gold filigree border, velvet back, ceremonial flip reveal.
export default function CardFace({ card, deck, revealed = false, onReveal, index = 0, clarifiers = [] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative cursor-pointer group" style={{ perspective: "1000px" }} onClick={handleReveal}>
        <div
        ...
        >
          ...
          >
            <div className="absolute inset-1.5 rounded-md"
              style={{ border: `1px solid ${accent}66` }} />
            <span style={{ fontSize: 44, color: accent, textShadow: `0 0 14px ${accent}99` }}
              className="opacity-80 group-hover:opacity-100 transition-opacity">
              {deck?.backGlyph || "✦"}
            </span>
            <div className="absolute inset-0 rounded-lg opacity-30 group-hover:opacity-60 transition-opacity"
              style={{ background: "linear-gradient(115deg, transparent 30%, rgba(255,255,255,0.18) 50%, transparent 70%)" }} />
          </div>

          {/* Face */}
          <div
            className="absolute inset-0 rounded-lg flex flex-col items-center justify-between p-2 text-center"
            style={{
              backfaceVisibility: "hidden",
          ...
          >
            ...
            <div className="flex-1 flex items-center justify-center">
              <span style={{ fontSize: 30, color: accent, textShadow: `0 0 12px ${accent}aa` }}>
                {isMajor ? "✶" : card?.card?.suit === "Cups" ? "♥" : card?.card?.suit === "Wands" ? "♣" : card?.card?.suit === "Swords" ? "♠" : "♦"}
              </span>
            </div>
      ...
      {clarifiers?.length > 0 && (
        <div className="flex gap-1.5 items-start">
          ...
          {clarifiers.map((c, i) => (
            <div key={i} className="relative rounded" style={{ width: 38, height: 62, transform: "rotate(0deg)" }}>
