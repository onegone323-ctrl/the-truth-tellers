...
export default function RecurringCardsChart({ data }) {
  ...
  return (
    <div className="lux-card rounded-xl p-4 sm:p-5">
      <div className="text-sm uppercase tracking-widest text-gold-leaf/80 mb-1">What Keeps Popping Up</div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(220, top.length * 32)}>
        <BarChart data={top} layout="vertical" margin={{ top: 5, right: 12, left: 10, bottom: 0 }}>
          ...
          <XAxis type="number" allowDecimals={false} tick={{ fill: "#a8a29e", fontSize: 14 }} stroke="rgba(212,175,55,0.2)" />
          <YAxis type="category" dataKey="name" width={130} tick={{ fill: "#d4c8a8", fontSize: 14 }} stroke="rgba(212,175,55,0.2)" />
          <Tooltip
